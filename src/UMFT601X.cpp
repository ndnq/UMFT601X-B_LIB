...
FT_STATUS readFromPipe(FT_HANDLE ftHandle, unsigned char pipeId, unsigned char* pdata) {
    ...
    ftStatus = FT_ReadPipe(ftHandle, pipeId, pdata, 1, &bytesRead, NULL);
    if (ftStatus != FT_OK) {
...
void writeBufferToPipe(FT_HANDLE ftHandle, unsigned char pipeId, unsigned char* buffer,unsigned int size) {
    ...
    ftStatus = FT_WritePipe(ftHandle, pipeId, buffer, size, &bytesWritten, NULL);
    if (ftStatus != FT_OK) {
        std::cout << "FT_WritePipe failed" << std::endl;
    } else {
...
void readBufferFromPipe(FT_HANDLE ftHandle, unsigned char pipeId, unsigned char* buffer,unsigned int size) {
    ...
    ftStatus = FT_ReadPipe(ftHandle, pipeId, buffer, size, &bytesRead, NULL);
    if (ftStatus != FT_OK) {
        std::cout << "FT_ReadPipe failed" << std::endl;
    } else {
